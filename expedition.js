import {safePoint,lavaAt,riftAt} from './world.js?v=9';
const TAU=Math.PI*2;
export function strike(g,x,y,r=60,damage=55,warning=1.6){g.hazards.push({kind:'eruption',x,y,r,life:warning,max:warning,damage});}
export function spawnOverlord(g){const e=g.spawn('bulwark',false,true),m=Math.max(0,g.time/60-10);e.lateBoss=true;e.hp=e.maxHp=(95000+m*18000+m*m*1400)*(g.region==='cinder'?1.25:1);e.title={ashwood:'Rootbound Titan',hollow:'Rift Matriarch',cinder:'Furnace Tyrant'}[g.region];e.r=40;e.speed=52+Math.min(30,m*1.5);e.attack=3;e.fury=false;g.nextBoss=g.time+180;g.bossSpawned=true;g.emit('warning',{text:e.title+' awakens · Watch the ground'});return e;}
export function attackOverlord(g,e){e.fury=e.hp<e.maxHp*.5;const p=g.player,damage=55+g.time/60*1.5,r=g.region==='cinder'?70:55;strike(g,p.x,p.y,r,damage,1.6);const n=e.fury?4:2;for(let i=0;i<n;i++){const a=i*TAU/n+g.time*.3;strike(g,p.x+Math.cos(a)*145,p.y+Math.sin(a)*145,r,damage,1.9);}e.attack=e.fury?3.8:5.5;g.burst(e.x,e.y,90,e.fury?'#fa7564':'#c1a0ff',.7);}
