...
export function buySpell(save,id,trackId){ensureProgression(save);const t=spellTracks(id).find(t=>t.id===trackId);if(!t||!WEAPONS.some(w=>w.id===id)&&!HEALTH_SPELLS.includes(id)||!HEALTH_SPELLS.includes(id)&&!weaponUnlocked(save,id))return false;const n=save.spellcraft[id]?.[t.id]||0,cost=spellCost(t,n);if(n>=t.max||save.gold<cost)return false;save.gold-=cost;save.spellSpent+=cost;save.spellcraft[id]??={};save.spellcraft[id][t.id]=n+1;return true;}
const ODDS=[{m:0,w:[60,27,10,2.5,.45,.049,.001]},{m:5,w:[35,30,23,9,2.5,.45,.05]},{m:10,w:[18,24,30,20,6.5,1.35,.15]},{m:20,w:[6,14,27,32,16,4.3,.7]},{m:30,w:[2,8,20,35,25,8,2]},{m:60,w:[0,2,10,28,38,17,5]}];
export function lootProfile(context=false){const c=context===true?{source:'boss'}:context&&typeof context==='object'?context:{},source=['boss','overlord','guardian','eclipse'].includes(c.source)?c.source:'ancient',hp=Number.isFinite(c.bossHp)?Math.max(0,c.bossHp):0;return {source,bonus:({ancient:0,boss:3,overlord:8,guardian:12,eclipse:25})[source]+Math.min(15,Math.max(0,Math.log10(Math.max(1,hp)/10000)*5)),floor:source==='eclipse'?5:source==='guardian'?(hp>=250000?4:3):source==='overlord'?2:source==='boss'?1:0};}
export function rarityOdds(seconds,context=false){const profile=lootProfile(context),m=Math.max(0,Math.min(60,(Number.isFinite(seconds)?seconds:0)/60+profile.bonus)),hi=ODDS.findIndex(o=>o.m>=m);let odds;if(hi<=0)odds=[...ODDS[0].w];else{const a=ODDS[hi-1],b=ODDS[hi],t=(m-a.m)/(b.m-a.m);odds=a.w.map((v,i)=>v+(b.w[i]-v)*t);}const total=odds.slice(profile.floor).reduce((a,b)=>a+b,0);return odds.map((v,i)=>i<profile.floor?0:v/total*100);}
export const gearChance=(seconds,boss=false)=>boss?1:Math.min(.65,.35+Math.max(0,seconds)/6000);
export function rollGear(seconds,context,random){const profile=lootProfile(context);if(random()>=gearChance(seconds,profile.source!=='ancient'))return null;const odds=rarityOdds(seconds,context);let roll=random()*100,tier=6;for(let i=0;i<odds.length;i++){roll-=odds[i];if(roll<0){tier=i;break;}}const family=FAMILIES[Math.min(FAMILIES.length-1,Math.floor(random()*FAMILIES.length))];return {id:`${family.id}:${tier}`,level:0};}
