...
const save=freshSave();delete save.settings.music;delete save.settings.musicVolume;save.gold=852;save.inventory={'flame:5':{level:8}};save.equipment={ranger:'flame:5'};save.spellcraft={flame:{power:8,opening:2}};const g=new Game(save);g.time=843;g.player.hp=64;g.skills={bolt:8,flame:6};g.hazards=hazards;g.spawn('brute');const snap=g.snapshot();const r=new Game(save,()=>{},snap);assert.equal(r.time,g.time);assert.equal(r.player.hp,g.player.hp);assert.deepEqual(r.skills,g.skills);assert.deepEqual(r.enemies,g.enemies);assert.deepEqual(r.hazards,g.hazards);assert.deepEqual(r.gear,g.gear);assert.deepEqual(r.spellcraft,g.spellcraft);assert.equal(save.gold,852);
checks.push('Legacy upgraded run restores health, gear, spellcraft, enemies, pending attacks and gold without reset');
