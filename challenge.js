import {SPECIAL_STAGES,eclipseBuild} from './chronicle.js?v=19';
const TAU=Math.PI*2;
...
export function spawnReaper(g){if(g.reaperSpawned)return;g.reaperSpawned=true;const e=g.spawn('nightreaper',false,true);e.reaper=true;e.title='The Last Eclipse';e.hp=e.maxHp=3000000;e.speed=165;e.r=38;e.attack=2;e.punishAt=g.time+15;e.shatterAt=0;g.emit('warning',{text:'THE LAST ECLIPSE · An evolved covenant can break its ward'});}
export function reaperDamage(g,e,damage,weapon){const build=eclipseBuild(g);if(!build)return 0;let total=damage*8;if(g.time>=(e.shatterAt||0)&&['chrono','aegis','solar','beam','blood','scythe'].includes(weapon)){total+=e.maxHp*.006;e.shatterAt=g.time+1;}return Math.min(total,e.maxHp*.025);}
function stepReaper(g,e,dt,dx,dy,d){const build=eclipseBuild(g);e.attackLabel=build?build.toUpperCase()+' · WARD BROKEN':'WARD IMMUNE · SEE UNLOCKS FOR THREE COVENANTS';e.attackUntil=g.time+2;e.slow=0;e.hp=Math.min(e.maxHp,e.hp+(build?0:e.maxHp*.08*dt));
 if(e.attack<=0){const p=g.player,fury=e.hp<e.maxHp*.5;for(let j=0;j<(fury?5:3);j++)strike(g,p.x+g.lastDir.x*j*95,p.y+g.lastDir.y*j*95,52,build?Math.max(50,g.maxHp()*.25):500,1.5+j*.25);e.attack=fury?3.5:5;}
 return (build?110:165)+(e.hp<e.maxHp*.5?20:0);
}
export function fireLostWeapon(g,id){const rank=g.rank(id),ev=g.evolved.includes(id),p=g.player,target=g.nearest(),area=g.area();if(!['aegis','chrono','torrent','blood','solar','void'].includes(id))return false;
 ...
 if(id==='chrono'){const r=(140+rank*15)*(ev?1.4:1)*area;g.aoe(p.x,p.y,r,32+rank*12,id);for(const e of g.enemies)if(Math.hypot(e.x-p.x,e.y-p.y)<r)e.slow=Math.max(e.slow,ev?5:2);g.burst(p.x,p.y,r,'#ffe1a0',.6);g.cd[id]=(ev?2.5:4)*g.haste(id);}
