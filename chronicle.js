...
export const EXTRA_WEAPONS=[
 ...
 {id:'chrono',name:'Clock needle',icon:'◷',color:'#ffe1a0',desc:'A clock pulse damages and slows nearby enemies.',base:'32 damage · slows for 2s',upgrade:'Stronger, wider time pulses.',evo:'Thirteenth hour',pair:'haste',evoDesc:'A great time pulse. Pair with Everglass mantle to break the Last Eclipse ward.',unlock:'Survive 10 minutes in Clockwork Spire'},
 {id:'blood',name:'Crimson bloom',icon:'❀',color:'#f3a7d4',desc:'A siphoning bloom strikes your nearest enemy and restores a little health.',base:'42 damage · heals 1 every 2s',upgrade:'More damage, healing and targets.',evo:'Heart of midnight',pair:'regen',evoDesc:'A wide siphon. Pair with Reaper’s orbit and Springwater rank 3 to break the Eclipse ward.',unlock:'Survive 10 minutes in Briarheart Garden'},
 {id:'solar',name:'Solar writ',icon:'☀',color:'#ffe499',desc:'Four radiant bolts fan toward the horde with deep piercing.',base:'38 damage · 4 bolts every 2s',upgrade:'More piercing, damage and rays.',evo:'Judgment dawn',pair:'power',evoDesc:'Nine radiant rays. Pair with Aurora and Stone oath rank 3 to break the Eclipse ward.',unlock:'Survive 10 minutes in Sunforge Citadel'},
...
export function validChronicle(s){const obj=v=>v&&typeof v==='object'&&!Array.isArray(v);if(s.unlockedWeapons!==undefined&&(!Array.isArray(s.unlockedWeapons)||s.unlockedWeapons.length>22||s.unlockedWeapons.some(id=>!EXTRA_WEAPONS.some(w=>w.id===id))))return false;if(s.chronicle!==undefined){const c=s.chronicle;if(!obj(c)||c.version!==1||!Array.isArray(c.earned)||c.earned.length>UNLOCKS.length||c.earned.some(id=>!UNLOCKS.some(u=>u.id===id))||!obj(c.records)||Object.entries(c.records).some(([id,r])=>!['ashwood','hollow','cinder',...SPECIAL_STAGES.map(s=>s.id)].includes(id)||!obj(r)||Object.entries(r).some(([k,v])=>!['time','kills','level','bosses','sites','evolutions','reaper'].includes(k)||!Number.isFinite(v)||v<0||v>1e9)))return false;}return true;}
export function eclipseBuild(g){const e=id=>g.evolved.includes(id);if(e('chrono')&&e('aegis'))return 'Hourglass covenant';if(e('solar')&&e('beam')&&g.rank('guard')>=3)return 'Solar judgment';if(e('blood')&&e('scythe')&&g.rank('regen')>=3)return 'Midnight pact';return null;}
export const ECLIPSE_RECIPES=[['Hourglass covenant','Thirteenth hour + Everglass mantle','Clock needle 8 + Quickening 2; Frost aegis 8 + Stone oath 2; evolve both in chests.'],['Solar judgment','Judgment dawn + Aurora + Stone oath 3','Solar writ 8 + Cinder heart 2; Prism ray 8 + Quickening 2; evolve both in chests.'],['Midnight pact','Heart of midnight + Reaper’s orbit + Springwater 3','Crimson bloom 8 + Springwater 2; Reaping crescent 8 + Windstep 2; evolve both in chests.']];
